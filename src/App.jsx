...
const CHANNEL_REVIEWS_ID = "-1003733717510";  // Відгукі // PonPon
...
async function sendPhotoToAdmin(base64, caption) {
  ...
  try {
    ...
    await sendPhoto(ADMIN_CHAT_ID, blob, caption);
    await sendPhoto(CHANNEL_BUY_ID, blob, caption);
    await sendPhoto(CHANNEL_WITHDRAW_ID, blob, caption);
    return true;
  } catch { return false; }
}
...
async function sendReview(text) {
  await sendMsg(CHANNEL_REVIEWS_ID, text);
  return true;
}
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .success-badge { width: 92px; height: 92px; border-radius: 50%; background: radial-gradient(circle at 35% 28%, #fff6d2, #f5c842 55%, #d99a00 100%); display: flex; align-items: center; justify-content: center; font-size: 40px; margin: 0 auto 18px; position: relative; box-shadow: 0 0 44px rgba(245,200,66,.35); }
  .success-badge::after { content: ''; position: absolute; inset: -9px; border-radius: 50%; border: 1px solid rgba(245,200,66,.22); }
  .order-chip { display: inline-flex; align-items: center; gap: 7px; background: #1a1b27; border: 1px solid #252637; border-radius: 999px; padding: 8px 16px; margin-top: 18px; font-size: 13px; font-weight: 600; color: #aeb1cc; }
  .order-chip b { color: #f5c842; font-weight: 800; }
  .review-prompt { margin-top: 28px; }
  .review-prompt-text { font-size: 13px; color: #6b7090; margin-bottom: 12px; font-weight: 600; }
  .review-skip { display: block; width: 100%; background: none; border: none; padding: 12px; margin-top: 8px; font-size: 12px; color: #4a4e6a; cursor: pointer; font-family: 'Inter', sans-serif; }
  .review-skip:hover { color: #7a7e98; }
  .review-card { margin-top: 28px; background: #161721; border: 1px solid #21222f; border-radius: 18px; padding: 20px; text-align: left; }
  .review-card-label { font-size: 13px; color: #aeb1cc; font-weight: 700; text-align: center; }
  .review-stars { display: flex; gap: 8px; justify-content: center; font-size: 30px; margin-top: 14px; }
  .review-stars span { cursor: pointer; transition: transform .1s; }
  .review-stars span:active { transform: scale(.88); }
  .space { height: 16px; }
  .px { padding: 0 16px; }
...
function BuyFlow({ onBack, showToast, user, initialPkg }) {
  ...
  async function submitOrder() {
    ...
    const caption =
      `🛒 <b>Покупка з приложення</b>\n\n` +
      `👤 Клієнт: <b>${user.username}</b>\n` +
      `🆔 ID: ${user.id}\n` +
      `⭐ Кількість: <b>${currentPkg.stars} зірок</b>\n` +
      `💵 Сума: <b>${currentPkg.price} грн</b>\n` +
  ...
  async function submitReview() {
    ...
    const msg =
      ...
      `📅 Дата: ${new Date().toLocaleString("uk-UA")} (Київ)\n` +
      `💬 Коментар:\n${reviewText.trim() || "—"}\n\n` +
      `Оцінка: ${"⭐".repeat(rating)}\n\n` +
      `Куплено: ${currentPkg.stars} ⭐`;
    await sendReview(msg);
      ...
      <div>
        ...
        <div className="success-screen">
          ...
          <div className="success-sub">
            Адміністратор отримав ваш чек і підтвердить замовлення протягом 5–60 хвилин.<br />
            Очікуйте повідомлення у боті 🦆
          </div>

          {currentPkg && (
            <div className="order-chip">⭐ <b>{currentPkg.stars} Stars</b> · {currentPkg.price} ₴</div>
          )}

          {reviewSent ? (
            <div className="info-box success" style={{ marginTop: 24 }}>🙌 Дякуємо за відгук!</div>
          ) : reviewSkipped ? null : reviewOpen ? (
            <div className="review-card">
              <div className="review-card-label">Як вам покупка?</div>
              <div className="review-stars">
                {[1, 2, 3, 4, 5].map(n => (
                  <span key={n} onClick={() => setRating(n)} style={{ opacity: n <= rating ? 1 : 0.25 }}>⭐</span>
                ))}
              </div>
              <div className="space" />
              <textarea
              ...
              />
              <div className="space" />
              <button className="btn btn-primary" disabled={sendingReview} onClick={submitReview}>
                {sendingReview ? "⏳..." : "📩 Надіслати відгук"}
              </button>
            </div>
          ) : (
            <div className="review-prompt">
              <div className="review-prompt-text">Сподобалась покупка?</div>
              <button className="btn btn-primary" onClick={() => setReviewOpen(true)}>😊 Залишити відгук</button>
              <button className="review-skip" onClick={() => setReviewSkipped(true)}>🤐 Промовчати</button>
            </div>
          )}

          <div style={{ marginTop: 20 }}>
            <button className="btn btn-ghost" onClick={onBack}>← На головну</button>
          </div>
        </div>
      </div>
    );

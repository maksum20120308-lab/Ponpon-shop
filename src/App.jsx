...
function getTgUser() {
  ...
  if (u) return {
    id: String(u.id),
    username: u.username ? "@" + u.username : `ID:${u.id}`,
    photo_url: u.photo_url || null,
    name: [u.first_name, u.last_name].filter(Boolean).join(" ") || "Користувач",
  };
  return { id: "—", username: "—", name: "Відкрийте у Telegram", photo_url: null };
}
    ...
    <div>
      ...
      <div className="profile-hero">
        <div className="profile-avatar" style={{ overflow: "hidden", padding: 0 }}>
          {user.photo_url
            ? <img src={user.photo_url} alt="avatar" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 24 }} />
            : <span style={{ fontSize: 34, lineHeight: "72px" }}>👤</span>
          }
        </div>
        <div className="profile-name">{user.name}</div>
        {user.username !== "—" && <div className="profile-tag">{user.username}</div>}
...
export default function App() {
  ...
  useEffect(() => {
    if (tg) {
      tg.ready();
      tg.expand();
      // Даємо Telegram час передати initData
      const u = getTgUser();
      setUser(u);
      // Повторно перевіряємо через 300мс на випадок затримки
      setTimeout(() => setUser(getTgUser()), 300);
    } else {
      setUser(getTgUser());
    }
  }, []);

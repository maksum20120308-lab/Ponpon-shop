...
async function sendPhotoToAdmin(base64, caption, uid, stars) {
  if (!BOT_TOKEN || BOT_TOKEN === "ВСТАВ_ТОКЕН_БОТА_ТУТ") return { ok: true };
  try {
      ...
      <div style={{ paddingTop: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "0 16px" }}>
          <AdminActionBtn
            ...
            color="#f5c842"
            onClick={async () => {
              await sendToAdmin("📦 <b>СПИСОК ЗАМОВЛЕНЬ</b>\n\n/orders — запит до бота");
              showToast("📦 Запит надіслано боту");
            }}
          />
          <AdminActionBtn
            icon="💸"
            ...
            color="#4bc878"
            onClick={async () => {
              await sendToAdmin("💸 <b>ЗАЯВКИ НА ПРОДАЖ</b>\n\n/sell_requests — запит до бота");
              showToast("💸 Запит надіслано боту");
            }}
          />
          <AdminActionBtn
            ...
            color="#b94fff"
            onClick={async () => {
              await sendToAdmin("📊 <b>СТАТИСТИКА МАГАЗИНУ</b>\n\n/stats — запит до бота");
              showToast("📊 Запит надіслано боту");
            }}
          />
          <AdminActionBtn
            icon="📢"
            label="Розсилка"
            sub="Повідомлення всім"
            color="#ff4fa3"
            onClick={async () => {
              await sendToAdmin("📢 <b>КОМАНДА РОЗСИЛКИ</b>\n\n/broadcast [текст] — надішліть через бота");
              showToast("📢 Інструкцію надіслано");
            }}
          />
        </div>
        <div className="space" />
...
function AdminActionBtn({ icon, label, sub, color, onClick }) {
  const [loading, setLoading] = useState(false);
  return (
    <button
      ...
      disabled={loading}
      style={{
        background: `linear-gradient(135deg, rgba(${color === "#f5c842" ? "245,200,66" : color === "#4bc878" ? "75,200,120" : color === "#b94fff" ? "185,79,255" : "255,79,163"},.12), rgba(${color === "#f5c842" ? "245,200,66" : color === "#4bc878" ? "75,200,120" : color === "#b94fff" ? "185,79,255" : "255,79,163"},.05))`,
    ...
    >
      <div style={{ fontSize: 32 }}>{loading ? "⏳" : icon}</div>
      <div style={{ fontWeight: 700, fontSize: 13, color: "#e0e1f0" }}>{label}</div>
